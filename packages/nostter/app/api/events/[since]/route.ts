...
export async function GET(request: NextRequest, { params: { since: sinceString } }: { params: { since?: string } }) {
	const url = new URL(request.url);

	const since = sinceString ? Number.parseInt(sinceString, 10) : undefined;
